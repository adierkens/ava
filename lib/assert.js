	...
	const assertions = {
		...
		deepEqual(actual, expected, message) {
			...
			if (result.pass) {
				...
				// TODO: Reuse descriptors (currently does not work due to a Kathryn bug)
				...
				const diff = formatAssertError.formatDescriptorDiff(actualDescriptor, expectedDescriptor);
				const values = diff ? [diff] : [
					formatAssertError.formatDescriptorWithLabel('Actual:', actualDescriptor),
					formatAssertError.formatDescriptorWithLabel('Must be deeply equal to:', expectedDescriptor)
				];
		...
		notDeepEqual(actual, expected, message) {
			...
			if (result.pass) {
				...
				fail(this, new AssertionError({
					...
					message,
					values: [formatAssertError.formatDescriptorWithLabel('Value is deeply equal:', actualDescriptor)]
				}));
			} else {
				pass(this);
			}
